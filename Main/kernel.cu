...
{
	sim->win[0] = 0;
	for (int j = 0; j < (sim->columns[0] - 3); j++)
	{
		for (int i = 0; i < sim->rows[0]; i++)
		{
			if (sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + 1] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + 2] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + 3] && (sim->board[i*sim->columns[0] + j] == 1 || sim->board[i*sim->columns[0] + j] == -1))
			{
				sim->win[0] = sim->board[i*sim->columns[0] + j];
			}
		}
	}
	for (int j = 0; j < (sim->columns[0]) / 2; j++)
	{
		for (int i = 0; i < sim->rows[0] - 3; i++)
		{
			if (sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + sim->columns[0]] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j +sim->columns[0] * 2] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j  +sim->columns[0] * 3] && (sim->board[i*sim->columns[0] + j] == 1 || sim->board[i*sim->columns[0] + j] == -1))
			{
				sim->win[0] = sim->board[i*sim->columns[0] + j];
			}
		}
	}
	for (int j = 0; j < (sim->columns[0] - 3) / 2; j++)
	{
		for (int i = 0; i < sim->rows[0] - 3; i++)
		{
			if (sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + sim->columns[0]] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + sim->columns[0] * 2] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j + sim->columns[0] * 3] && (sim->board[i*sim->columns[0] + j] == 1 || sim->board[i*sim->columns[0] + j] == -1))
	...
	for (int j = 0; j < (sim->columns[0] - 3) / 2; j++)
	{
		for (int i = sim->rows[0] + 3; i < sim->rows[0]; i++)
		{
			if (sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j - sim->columns[0]] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j - sim->columns[0] * 2] && sim->board[i*sim->columns[0] + j] == sim->board[i*sim->columns[0] + j - sim->columns[0] * 3] && (sim->board[i*sim->columns[0] + j] == 1 || sim->board[i*sim->columns[0] + j] == -1))
			{
				sim->win[0] = sim->board[i*sim->columns[0] + j];
...
{
	cudaMallocManaged((void **)&sim->board, sizeof(int) * 42);
	for (int i = 0; i < 42; i++)
	{
		sim->board[i] = 0;
	}
...
void possibleMoves(Simulation *sim)
{
	...
	for (int i = 0; i < sim->columns[0]; i++)
	{
		if (sim->board[i] == 0)
		{
			moves++;
			int count = i;
			while (sim->board[count] == 0 && count < sim->rows[0] * sim->columns[0])
			{
				count += sim->columns[0];
			}
			count -= sim->columns[0];
			sim->possible[moves] = count;
		}
	}
...
int main(void)
{
	...
	{ 
		...
		{
			int *board;
			cudaMallocManaged(&board, sizeof(int) * 42);
			for (int i = 0; i < 42; i++)
			{
				board[i] = sim[0]->board[i];
			}
			int *move;// = new int[42];
			cudaMallocManaged(&move, sizeof(int) * 42);
			for (int i = 0; i < 42; i++)
			{
				move[i] = 0;
			}
